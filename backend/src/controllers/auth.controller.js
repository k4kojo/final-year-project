...
/**
 * Generates a random 4-digit token between 1000 and 9999.
...
export const signUp = async (req, res) => {
  try {
    ...
    const verificationToken = generateFourDigitToken();
...
export const resendVerification = async (req, res) => {
  try {
    ...
    const newToken = generateFourDigitToken();
...
export const requestPasswordReset = async (req, res) => {
  try {
    ...
    const resetToken = generateFourDigitToken();
    const hashedToken = await bcrypt.hash(resetToken, 8);
    const expiryTime =
      NODE_ENV === "development"
        ? 1000 * 60 * 30 // 30 minutes for development
        : 1000 * 60; // 60 seconds for production
